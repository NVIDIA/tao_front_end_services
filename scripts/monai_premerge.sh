...
if [ -z "$NV_TAO_API_TOP"]; then
    echo "NV_TAO_API_TOP is not set"
    NV_TAO_API_TOP=$(pwd)
    echo "Using the current directory: $NV_TAO_API_TOP"
    # check if the current directory has "runtests.sh"
    if [ ! -f "$NV_TAO_API_TOP/runtests.sh" ]; then
...
# Copy the NV_TAO_API_TOP folder into the container
docker cp $NV_TAO_API_TOP premerge:/opt/tao-toolkit-apis
